...
class Solution(object):
    def minSwap(self, A, B):
        ...
        for i in range(1, N):
            ...
            if A[max_index - i] < B[max_index - i + 1] and B[max_index - i] < A[max_index - i + 1]:
                prop.append(swap[i - 1])
            no_swap[i] = min(prop)

            prop = []
            if B[max_index - i] < A[max_index - i + 1] and A[max_index - i] < B[max_index - i + 1]:
                prop.append(no_swap[i - 1])
            if B[max_index - i] < B[max_index - i + 1] and A[max_index - i] < A[max_index - i + 1]:
                prop.append(swap[i - 1])
            swap[i] = min(prop) + 1
        return min(swap[-1], no_swap[-1])

    def minSwap_1(self, A, B):
        """
        :type A: List[int]
        :type B: List[int]
        :rtype: int
