...
class Solution(object):
    def maxProfit(self, k, prices):
        ...
        def calc_max(start, end):
            max_1 = 0
            stack = []
            for i in range(start, end + 1):
                a = self.parents[i]
                while stack and stack[-1][0] > prices[a[0]]:
                    stack.pop()
                if not stack:
                    stack.append([prices[a[0]], prices[a[1]]])
                else:
                    stack[-1][-1] = max(prices[a[1]], stack[-1][-1])
                a = stack[-1]
                max_1 = max(max_1, a[1] - a[0])
            # print(stack)
            return max_1
        # print(calc_max(1, 1))
        self.max_1 = 0
        ...
        def dfs(start, end, k):
            if start > end:
                return 0
            if (start, end, k) in self.dict:
                return self.dict[(start, end, k)]
            if k == 1:
                m = calc_max(start, end)
                self.max_1 = max(self.max_1, m)
                self.dict[(start, end, k)] = m
                return calc_max(start, end)
            m = 0
            for i in range(start, end + 1):
                for j in range(1, k):
                    a = dfs(start, i, j) + dfs(i + 1, end, k - j)
                    m = max(a, m)
                # self.max_1 = max(self.max_1, a)
            self.dict[(start, end, k)] = m
            self.max_1 = max(self.max_1, m)
            return self.dict[(start, end, k)]
        a = dfs(0, len(parents) - 1, k)
        print(self.dict)
        return self.max_1
    ...
    def maxProfit_1(self, k, prices):
        ...
        if k >= len(parents):
            sum = 0
            for i in parents:
                sum += prices[i[1]] - prices[i[0]]
            return sum
        hold = [[0] * l for _ in range(k + 1)]
        unhold = [[0] * l for _ in range(k + 1)]
        hold[0][0] = -prices[0]
        for j in range(1, l):
        ...
        for i in range(1, k + 1):
            for j in range(1, l):
                hold[i][j] = max(unhold[i][j - 1] - prices[j], hold[i][j - 1])
    ...
    def maxProfit_3(self, k, prices):
        ...
        while 1:
            ...
            if p <= v:
                break
            while vp_pair and v <= vp_pair[-1][0]:
                profit.append(vp_pair[-1][1] - vp_pair[-1][0])
                del vp_pair[-1]
            while vp_pair and p > vp_pair[-1][1]:
                profit.append(vp_pair[-1][1] - v)
                v = vp_pair[-1][0]
                del vp_pair[-1]
            vp_pair.append([v, p])
            # print i,vp_pair,profit
        for v, p in vp_pair:
            profit.append(p - v)
        # print vp_pair,profit
        heapq.heapify(profit)
        ans = sum(heapq.nlargest(k, profit))
        return ans


print(Solution().maxProfit_1(3,[6,4,4,5,1,2,4,6,7,6,5,7,8,0,1]))
print(Solution().maxProfit_1(1,[1,2]))
k = 29
...
print(Solution().maxProfit_1(k, prices))